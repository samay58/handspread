...
def _extract_value(sec_metrics: dict[str, Any], key: str) -> tuple[float | None, Any]:
    """Get numeric value and source object from sec_metrics."""
...
def compute_multiples(
    ...
    result["ev_revenue"] = _safe_divide(
        ev_val, rev_val, "ev_revenue", "enterprise_value / revenue",
        ev_bridge.enterprise_value, rev_src,
    )

    ebitda_val, ebitda_src = _extract_value(sec_metrics, "ebitda")
    result["ev_ebitda"] = _safe_divide(
        ev_val, ebitda_val, "ev_ebitda", "enterprise_value / ebitda",
        ev_bridge.enterprise_value, ebitda_src,
    )

    ebit_val, ebit_src = _extract_value(sec_metrics, "operating_income")
    result["ev_ebit"] = _safe_divide(
        ev_val, ebit_val, "ev_ebit", "enterprise_value / operating_income",
        ev_bridge.enterprise_value, ebit_src,
    )

    fcf_val, fcf_src = _extract_value(sec_metrics, "free_cash_flow")
    result["ev_fcf"] = _safe_divide(
        ev_val, fcf_val, "ev_fcf", "enterprise_value / free_cash_flow",
        ev_bridge.enterprise_value, fcf_src,
    )
    ...
    ni_val, ni_src = _extract_value(sec_metrics, "net_income")
    result["pe"] = _safe_divide(
        mcap_val, ni_val, "pe", "market_cap / net_income",
        market.market_cap, ni_src,
    )

    bv_val, bv_src = _extract_value(sec_metrics, "stockholders_equity")
    result["price_book"] = _safe_divide(
        mcap_val, bv_val, "price_book", "market_cap / stockholders_equity",
        market.market_cap, bv_src,
    )

    # FCF yield = free_cash_flow / market_cap
    result["fcf_yield"] = _safe_divide(
        fcf_val, mcap_val, "fcf_yield", "free_cash_flow / market_cap",
        fcf_src, market.market_cap,
    )

    # Dividend yield = dividends_per_share / price
    dps_val, dps_src = _extract_value(sec_metrics, "dividends_per_share")
    result["dividend_yield"] = _safe_divide(
        dps_val, price_val, "dividend_yield", "dividends_per_share / price",
        dps_src, market.price,
    )
